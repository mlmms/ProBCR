...
# Copies them into another folder, separating it from T2, ADC, sbXXXXX, etc
...
dst="/Users/monicasilva/Documents/1_Original_data/DWI_sequences_cohort2"
                            ...
                            if filename.startswith('I'):
                                ...
                                for image, fields in ids.items():
                                    ...
                                    if series_description.startswith("dif") or series_description.startswith("ddif") or "dwi" in series_description or series_description.startswith("b1"):
                                        ...
                                        csv_deid_coding.append([aux_str, series_description, b_value, slice_thickness, repetition_time, echo_time, field_strength])
                                        print(csv_deid_coding)
                                        shutil.copytree(os.path.join(src,patientDir,directorySequence), os.path.join(dst,aux_str))

                                        count_patientDir += 1

                                        with open(os.path.join(dst, "DWI_parameters" + ".csv"), "a") as f:
                                            writer = csv.writer(f, delimiter='/')
                                            writer.writerow([aux_str, series_description, b_value, slice_thickness, repetition_time, echo_time, field_strength])
                                    break
                                break
                            break
                        break
