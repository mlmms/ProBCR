...
BASE <- "/Users/monicasilva/Documents/Thesis_Writing/4_Results/4_Results_Normalization_II/2_PREOPERATIVE/PLII_seedingPAIRS_SMOTE_"
...
for (i in 1:length(pmaxs_value)) {
  for (j in 1:length(SEED)) { #length(SEED)
    ...
    results_path <- paste0(BASE,"PreOperative_PLII_SMOTEoversampling.xlsx")
    ...
    index_clin <- which(grepl("Clin_|HPBiopsy" , colnames(df_train_pred_SMOTE)))
