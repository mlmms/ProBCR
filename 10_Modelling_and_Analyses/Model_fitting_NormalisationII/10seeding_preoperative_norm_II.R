...
BASE <- "/Users/monicasilva/Documents/Thesis_Writing/4_Results/4_Results_Normalization_II/2_PREOPERATIVE/PLII_seedingPAIRS"
...
for (i in 1:length(pmaxs_value)) {
  for (j in 1:length(SEED)) { #length(SEED)
    ...
    # Data Loading -------------------------------------------------------------
    clin_path <- "/Users/monicasilva/Documents/1_Original_data/Clinical_data/ProBCR_2020_preOPERATIVE_clinical_variables_93p_simple.csv"
    ...
    cols_to_factor <- colnames(clin_data[,c(1:10)])
    cols_to_numeric <- colnames(clin_data[,c(11:13)])
    ...
    clin_continuous_variables <- clin_data[,c(11:13)]
    UID <- clin_data[,1]
    Label <- clin_data[,2]
    ...
    Clin_preopPSA.10 <- cbind(UID, Clin_preopPSA.10)
    
    HPBiopsy_highest_GS1 <- clin_data[,7]
    HPBiopsy_highest_GS1 <- as.data.frame(HPBiopsy_highest_GS1)
    HPBiopsy_highest_GS1 <- cbind(UID, HPBiopsy_highest_GS1)
    ...
    #to_one_hot_df <- clin_data[,c(3,4,6,8,9,10)]
    to_one_hot_df <- clin_data[,c(3:10)]
    ...
    #clin_categorical_variables_onelevel <- cbind(UID,clin_categorical_variables_onelevel)
    
    
    #clin_data <- clin_data[,-c(3,4,6,8,9,10)]
    clin_data <- clin_data[,-c(3:10)]
    clin_df_ii <- merge(clin_data, clin_categorical_variables_multiplelevels_onehotencoded, by = 'UID')
    #clin_df_ii <- merge(clin_df_ii, Clin_preopPSA, by = 'UID')
    
    #Clinical data: encode
    cols_to_factor <- colnames(clin_df_ii[,c(1,2,6:30)])
    cols_to_numeric <- colnames(clin_df_ii[,c(3,4,5)])
    ...
    #sapply(clin_df_ii, class)
    #sapply(clin_df_ii,summary)
    ...
    x_train_to_std <- shuffled_df_ii[trainIndex, c(3,4,5,(index_Label_y+1):length(shuffled_df_ii))]
    #features to not standardize (categorical variables)
    x_train_to_not_std <- shuffled_df_ii[trainIndex, -c(3,4,5,(index_Label_y+1):length(shuffled_df_ii))]
    
    #TEST
    #features to standardize (continuous variables)
    x_test_to_std <- shuffled_df_ii[-trainIndex, c(3,4,5,(index_Label_y+1):length(shuffled_df_ii))]
    #features to not standardize (categorical variables)
    x_test_to_not_std <- shuffled_df_ii[-trainIndex, -c(3,4,5,(index_Label_y+1):length(shuffled_df_ii))]
    ...
    #define outcome and predictors
    results_path <- 
      paste0(BASE,"PreOperative_PLII_nooversampling.xlsx")
    ...
    index_T2 <- which(grepl("T2ii" , colnames(df_train_pred)))
    index_b1000 <- which(grepl("b1000ii" , colnames(df_train_pred)))
    index_ADC <- which(grepl("ADCii" , colnames(df_train_pred)))
    index_clin <- which(grepl("Clin_|HPBiopsy" , colnames(df_train_pred)))
...
#define outcome and predictors
results_path <- 
  paste0(BASE,"PreBiopsy_PLII_nooversampling_",pl2_2,"_seed",SEED[j],".xlsx")
...
for (i in 1:dim(permutations)[1]) { #dim(permutations)[1]
  print(i)
  perm_i <- permutations[i,]
  #print(perm_i)
  
  set.seed(SEED[j])
  assign(paste0("pl", i), prioritylasso(X = df_train_pred, Y = df_train_out, family = "binomial",
                                        type.measure="auc", nfolds = 4, foldid = foldids,
                                        blocks = perm_i, standardize = FALSE, cvoffset=TRUE, cvoffsetnfolds = 4,
                                        lambda.type="lambda.min", max.coef = c(1,6,0,1) ))  #max.coef = c(3,3,3,3) ))
  
  varNames[i] <- paste0("pl", i) #adds priority lasso's object names to list
  
  #### AUC CALCULATION AND PLOTTING
  d <- get(varNames[i]) # copy the object to an object named 'd'
  
  d$nzero
  n_coeffs1[i] <- d$nzero[[1]]
  n_coeffs2[i] <- d$nzero[[2]]
  n_coeffs3[i] <- d$nzero[[3]]
  n_coeffs4[i] <- d$nzero[[4]]
  ...
  ##plotpl_roc_train)
  print(pl_roc_train)
  pl_roc_train_auc[i] <- pl_roc_train[["auc"]]
  ...
  max_F[i] <- round(OCP_confusion_matrix_train_lmin[["byClass"]][["F1"]],4)
  max_P[i] <- round(OCP_confusion_matrix_train_lmin[["byClass"]][["Precision"]],4)
  max_R[i] <- round(OCP_confusion_matrix_train_lmin[["byClass"]][["Recall"]],4)
  
  OCP_confusion_matrix_train_lmin_negative <- confusionMatrix(reference =y_train_num, data = as.factor(new_predictions_train_lmin), mode = "everything", positive = "0")
  print(OCP_confusion_matrix_train_lmin_negative)
  
  max_F0[i] <- round(OCP_confusion_matrix_train_lmin_negative[["byClass"]][["F1"]],4)
  max_P0[i] <- round(OCP_confusion_matrix_train_lmin_negative[["byClass"]][["Precision"]],4)
  max_R0[i] <- round(OCP_confusion_matrix_train_lmin_negative[["byClass"]][["Recall"]],4)
...
results <- cbind(varNames,aucsTrain,stdTrain,cisTrain_l,cisTrain_u,
                 aucsTest,stdTest,cisTest_l,cisTest_u,n_coeffs1,n_coeffs2, n_coeffs3, n_coeffs4, str, 
                 max_cvm, pl_roc_train_auc, max_F, max_P, max_R, max_F0, max_P0, max_F0)

results_df <- as.data.frame(results)
permutations_matrix <- matrix(unlist(names_permutations), nrow = dim(names_permutations)[1], ncol = dim(names_permutations)[2])

permutations_df <- as.data.frame(permutations_matrix)
results_permut4 <- cbind(permutations_df, results_df)

write_xlsx(results_permut4, results_path, col_names = TRUE, format_headers = FALSE)
...
results_path <- paste0(BASE,"PreBiopsy_PLII_SMOTEoversampling_",pl2_1_smote,"_seed",SEED[j],".xlsx")
...
for(i in 1:5){
  df_train_pred_SMOTE[,i]<- round(df_train_pred_SMOTE[,i],0)
}
...
for (i in 1:dim(permutations)[1]) { #dim(permutations)[1]
  ...
  set.seed(SEED[j])
  assign(paste0("pl", i), prioritylasso(X = df_train_pred_SMOTE, Y = df_train_out_SMOTE, family = "binomial",
                                        type.measure="auc", nfolds = 4, foldid = foldids,
                                        blocks = perm_i, standardize = FALSE, cvoffset=TRUE, cvoffsetnfolds = 4,
                                        lambda.type="lambda.min", max.coef = c(1,7,0,1) ))  #pmax = 10 ) ) #  max.coef = c(3,3,3,3)
...
results_SMOTE <- cbind(varNames,aucsTrain,stdTrain,cisTrain_l,cisTrain_u,
                       aucsTest,stdTest,cisTest_l,cisTest_u,n_coeffs1,n_coeffs2, n_coeffs3, n_coeffs4, str, 
                       max_cvm, pl_roc_train_auc, max_F, max_P, max_R, max_F0, max_P0, max_R0)

results_df_SMOTE <- as.data.frame(results_SMOTE)
permutations_matrix <- matrix(unlist(names_permutations), nrow = dim(names_permutations)[1], ncol = dim(names_permutations)[2])

permutations_df <- as.data.frame(permutations_matrix)
results_permutSMOTE <- cbind(permutations_df, results_df_SMOTE)

write_xlsx(results_permutSMOTE, results_path, col_names = TRUE, format_headers = FALSE)
...
results_path <- paste0(BASE,"PreBiopsy_PLII_SMOTEoversampling_",pl2_2_smote,"_seed",SEED[j],".xlsx")
...
for (i in 1:dim(permutations)[1]) { #dim(permutations)[1]
  ...
  assign(paste0("pl", i), prioritylasso(X = df_train_pred_SMOTE, Y = df_train_out_SMOTE, family = "binomial",
                                        type.measure="auc", nfolds = 4, foldid = foldids,
                                        blocks = perm_i, standardize = FALSE, cvoffset=TRUE, cvoffsetnfolds = 4,
                                        lambda.type="lambda.min", max.coef = c(3,3,3,0) ))  #pmax = 10 ) ) #  max.coef = c(3,3,3,3)
