...
BASE <- "/Users/monicasilva/Documents/Thesis_Writing/4_Results/4_Results_Normalization_II/3_POSTOPERATIVE/PLII_seedingPAIRS"
...
for (i in 1:length(pmaxs_value)) {
  for (j in 1:length(SEED)) { #length(SEED)
    ...
    # Data Loading -------------------------------------------------------------
    clin_path <- "/Users/monicasilva/Documents/1_Original_data/Clinical_data/ProBCR_2020_clinical_variables_93p_simple.csv"
    ...
    cols_to_factor <- colnames(clin_data[,c(1:28)])
    cols_to_numeric <- colnames(clin_data[,c(29:35)])
    ...
    clin_continuous_variables <- clin_data[,c(29:35)]
    UID <- clin_data[,1]
    Label <- clin_data[,2]
    ...
    to_one_hot_df <- clin_data[,c(3:28)]
    ...
    cols_to_factor <- colnames(clin_df_ii[,c(1,10:84)])
    cols_to_numeric <- colnames(clin_df_ii[,c(3:9)])
    ...
    x_train_to_std <- shuffled_df_ii[trainIndex, c(3:9,(index_Label_y+1):length(shuffled_df_ii))]
    #features to not standardize (categorical variables)
    x_train_to_not_std <- shuffled_df_ii[trainIndex, -c(3:9,(index_Label_y+1):length(shuffled_df_ii))]
    
    #TEST
    #features to standardize (continuous variables)
    x_test_to_std <- shuffled_df_ii[-trainIndex, c(3:9,(index_Label_y+1):length(shuffled_df_ii))]
    #features to not standardize (categorical variables)
    x_test_to_not_std <- shuffled_df_ii[-trainIndex, -c(3:9,(index_Label_y+1):length(shuffled_df_ii))]
    ...
    #y_test_num <- shuffled_df_ii[-trainIndex,2] # NEW
    y_test_num <- y_test[,2]
    ...
    #define outcome and predictors
    results_path <- 
      paste0(BASE,"PostOperative_PLII_nooversampling.xlsx")
    ...
    index_clin <- which(grepl("Clin_|HPBiopsy|Surgery|HPSpec" , colnames(df_train_pred)))
