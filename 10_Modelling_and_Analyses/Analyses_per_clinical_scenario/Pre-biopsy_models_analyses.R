...
ggplot(df4, aes(x=pmaxs_value, y=F_max, fill=pmaxs_value)) + 
  geom_boxplot() +
  ...
  ggtitle("Pre-Biopsy Lasso-penalised models' performance \n by maximum amount of features allowed") +
...
PB_Lasso_best_model <- LASSO_pmax9

# PRIORITY-LASSO I --------------------------------------------------------

# Model selection ---------------------------------------------------------
# Choosing between configurations for one algorithm
data_I <- PLI_NO_Norm_I_Prebio
...
friedman.test(df) #p-value = 0.002782
...
pl5 <- ggplot(df_PL_I, aes(x=pmaxs_value, y=max_F, fill=pmaxs_value)) + 
        geom_boxplot() + 
        ...
        ggtitle("Pre-Biopsy Priority-Lasso-2-blocks models' performance \n with priority sequence 'Clinical > Radiomics'") +
...
pl5$layers[[2]]$aes_params$textsize <- 3
pl5

# Conclusion

PB_PL_2blocks_best_model <- PL_I_pmax18

# Priority-LASSO_II -------------------------------------------------------

# Model selection ---------------------------------------------------------
# Choosing between configurations for one algorithm
data_I <- PLII_NO_Norm_I_Prebio
...
pl6 <- ggplot(df3, aes(x=Block_Priority, y=Fmax, fill = Highlighted)) + 
        geom_boxplot() +
        ...
        ggtitle("Pre-Biopsy Priority-Lasso-2 (4 blocks) performances by block priority sequence") +
        theme(plot.title = element_text(size = 10), legend.title = element_text(size = 9)) +
        #scale_x_discrete(labels=function(x){sub("\\s", "\n", x)}) +
...
pl6$layers[[2]]$aes_params$textsize <- 3
pl6
...
pl7 <- ggplot(data_pl19_I, aes(x=pmaxs_value, y=max_F, fill=pmaxs_value)) + 
        geom_boxplot() + 
        ...
        ggtitle("Comparison of performance of Pre-Biopsy Priority-Lasso-4-blocks models") +
        theme(plot.title = element_text(size = 8), legend.title = element_text(size = 8)) +
        #scale_x_discrete(labels=function(x){sub("\\s", "\n", x)}) +
        theme_classic() + #geom_jitter(color="black", size=0.4, alpha=0.9) +
        scale_fill_brewer(palette = 'Blues') +
...
pl7$layers[[2]]$aes_params$textsize <- 3
pl7
...
PB_PL_4blocks_best_model <- PL2_pmax1701


# Plot
PB_Lasso_best_model
PB_PL_2blocks_best_model
PB_PL_4blocks_best_model
...
best_LASSO_I <- PB_Lasso_best_model #pmax9
best_PL_I <-  PB_PL_2blocks_best_model #(1,8)
best_PL_II <- PB_PL_4blocks_best_model #(1,7,0,1)
...
my_comparisons = list( c("Lasso pmax=9", "Priority-Lasso-2-blocks max-coef = (1,8)"), c("Priority-Lasso-2-blocks max-coef =(1,8)", "Priority-Lasso-4-blocks max-coef = (1,7,0,1)"), c("Lasso pmax = 9", "Priority-Lasso-4-blocks max-coef = (1,7,0,1)") )

pl8 <- ggplot(df_best_Fmax, aes(x=Model, y=Fmax, fill = Model)) + 
        geom_boxplot() +
        theme(legend.position="none") +
        labs(fill = "") +
        xlab("Best performing Pre-biopsy models") + 
        ylab(expression(F[max])) +
        ggtitle("Pre-biopsy best performing models performance comparison") +
        #scale_fill_brewer(palette="BuPu") +
        scale_fill_manual(values=c("azure", "lightskyblue1", "steelblue3")) +
...
pl8$layers[[2]]$aes_params$textsize <- 3
pl8
