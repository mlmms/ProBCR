#Choosing best hyperparameters - Post-treatment
...
posttreatment_files <- list.files(path = "/Users/monicasilva/Documents/ANALYSIS/3_POSTOPERATIVE", 
...
# File Loading ------------------------------------------------------------
Lasso_NO_Norm_I_Posttreat <- read.xlsx(posttreatment_files[1])
#Lasso_SMOTE_Norm_I_Posttreat <- read.xlsx(posttreatment_files[3])
PLI_NO_Norm_I_Posttreat <- read.xlsx(posttreatment_files[5])
#PLI_SMOTE_Norm_I_Posttreat <- read.xlsx(posttreatment_files[7])
PLII_NO_Norm_I_Posttreat <- read.xlsx(posttreatment_files[9])
#PLII_SMOTE_Norm_I_Posttreat <- read.xlsx(posttreatment_files[11])
...
data_I <- Lasso_NO_Norm_I_Posttreat
# CLEANING
...
#PLOT
pl14 <- ggplot(df4, aes(x=pmaxs_value, y=F_max, fill=pmaxs_value)) + 
  geom_boxplot() +
  ...
  ggtitle("Post-Operative Lasso-penalised models' performance \n by maximum amount of features allowed") +
...
pl14$layers[[1]]$aes_params$textsize <- 3
pl14
...
# Model selection ---------------------------------------------------------
# Choosing between configurations for one algorithm
data_I <- PLI_NO_Norm_I_Posttreat
...
pl15 <- ggplot(df_PL_I, aes(x=pmaxs_value, y=max_F, fill=pmaxs_value)) + 
  geom_boxplot() + 
  ...
  ggtitle("Post-Operative Priority-Lasso-2-blocks models' performance \n with priority sequence 'Clinical > Radiomics'") +
...
pl15$layers[[1]]$aes_params$textsize <- 3
pl15

wilcox.test(PL_I_Fmax17, PL_I_Fmax18, paired = TRUE)
...
# Model selection ---------------------------------------------------------
# Choosing between configurations for one algorithm
data_I <- PLII_NO_Norm_I_Posttreat
...
pl16 <- ggplot(df3, aes(x=Block_Priority, y=Fmax, fill = Highlighted)) + 
          geom_boxplot() +
          scale_fill_manual(values=c("grey", "darkorange2")) +
          ...
          ggtitle("Post-Operative Priority-Lasso-2 (4-blocks) performances by priority sequence") +
          theme(plot.title = element_text(size = 10), legend.title = element_text(size = 9)) +
          #scale_x_discrete(labels=function(x){sub("\\s", "\n", x)}) +
          theme_classic() +
          theme(legend.position="none", plot.title = element_text(size = 9, hjust = 0.5), axis.title = element_text(size = 9)) +
...
pl16$layers[[2]]$aes_params$textsize <- 3
pl16
...
# Model selection ---------------------------------------------------------
# Choosing between configurations for one algorithm
PL_II <- PLII_NO_Norm_I_Posttreat
...
comparisons_PL_II_nemenyi_sign = list(c("1,7,0,1", "2,1,2,1")) # *


pl17 <- ggplot(data_pl19_I, aes(x=pmaxs_value, y=max_F, fill=pmaxs_value)) + 
  geom_boxplot() + 
  ...
  ggtitle("Comparison of performance of Post-Operative Priority-Lasso-4-blocks models") +
  theme(plot.title = element_text(size = 8), legend.title = element_text(size = 8)) +
  #scale_x_discrete(labels=function(x){sub("\\s", "\n", x)}) +
  theme_classic() + #geom_jitter(color="black", size=0.4, alpha=0.9) +
  scale_fill_brewer(palette = 'Oranges') +
...
pl17$layers[[2]]$aes_params$textsize <- 3
pl17
...
my_comparisons = list( c("Lasso pmax = 9", "Priority-Lasso-2-blocks max-coef = (1,8)"), 
                       c("Priority-Lasso-2-blocks max-coef = (1,8)", "Priority-Lasso-4-blocks max-coef = (1,7,0,1)"),
                       c("Lasso pmax = 9", "Priority-Lasso-4-blocks max-coef = (1,7,0,1)") )

pl18 <- ggplot(df_best_Fmax, aes(x=Model, y=Fmax, fill = Model)) + 
  geom_boxplot() +
  theme(legend.position="none") +
  labs(fill = "") +
  xlab("Best performing Post-Operative models") + 
  ylab(expression(F[max])) +
  ggtitle("Post-Operative best performing models comparison") +
  scale_fill_brewer(palette="Oranges") +
...
pl18$layers[[1]]$aes_params$textsize <- 3
pl18
