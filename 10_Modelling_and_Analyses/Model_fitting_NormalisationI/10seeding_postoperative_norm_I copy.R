...
BASE <- "/Users/monicasilva/Documents/Thesis_Writing/4_Results/4_Results_Normalization_I/3_POSTOPERATIVE/PLII_seeding123456789_SMOTE_"
...
for (i in 1:length(pmaxs_value)) {
  for (j in 1:length(SEED)) { #length(SEED)
    ...
    index_T2 <- which(grepl("T2i" , colnames(df_train_pred_SMOTE)))
    index_b1000 <- which(grepl("b1000i" , colnames(df_train_pred_SMOTE)))
    index_ADC <- which(grepl("ADCi" , colnames(df_train_pred_SMOTE)))
    index_clin <- which(grepl("Clin_|HPBiopsy|Surgery|HPSpec" , colnames(df_train_pred_SMOTE)))
    ...
    for (p in 1:dim(permutations)[1]) { #dim(permutations)[1]
      ...
      assign(paste0("pl", p), prioritylasso(X = df_train_pred_SMOTE, Y = df_train_out_SMOTE, family = "binomial",
                                            type.measure="auc", nfolds = 4, foldid = foldids,
                                            blocks = perm_i, standardize = FALSE, cvoffset=TRUE, cvoffsetnfolds = 4,
                                            lambda.type="lambda.min", pmax = pmaxs_value[i] ))  #pmax = 10 ) ) #  max.coef = c(3,3,3,3)
      ...
      results_PLII <- rbind(results_PLII, data.frame(pmaxs_value[i], SEED[j],
                                                     "PL-II", varNames[p], permutations_matrix[p,1],
