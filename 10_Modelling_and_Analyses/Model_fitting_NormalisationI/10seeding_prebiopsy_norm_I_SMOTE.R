...
BASE <- "/Users/monicasilva/Documents/Thesis_Writing/4_Results/4_Results_Normalization_I/1_PREBIOPSY/PLII_seedingPAIRS_SMOTE_"
  ...
  for (j in 1:length(SEED)) { #length(SEED)
    ...
    for (p in 1:dim(permutations)[1]) { #dim(permutations)[1]
      ...
      results_PLII <- rbind(results_PLII, data.frame(pmaxs_value[i,1], pmaxs_value[i,2], pmaxs_value[i,3], pmaxs_value[i,4],
                                                     SEED[j],
                                                     "PL-II", varNames[p], 
                                                     permutations_matrix[p,1],
                                                     permutations_matrix[p,2],
                                                     permutations_matrix[p,3], 
