...
# ANALYSIS II : with background (prostate AND surrounding tissue)
...
def _createParameterFile(h_final):
    ...
    data = dict(
        ...
        setting=dict(
            normalize=False,
            binWidth=h_final,
            resampledPixelSpacing=[0.66, 0.66, 4.5],
            padDistance=5,
            distances=[1, 2, 3],
            correctMask=True,
            force2D=True,
...
# 0_Setting up data
src = "/Users/monicasilva/Documents/1_Original_data/Final_Cohort_2020_99p/pre_PyRadiomics/6.0_T2ax/Analysis_II(withbackground)/5_pyRadiomics_input"
dst = "/Users/monicasilva/Documents/1_Original_data/Final_Cohort_2020_99p/pre_PyRadiomics/6.0_T2ax/Analysis_II(withbackground)/6_pyRadiomics_output"

OUTPUTCSV = os.path.join(dst, "results_2020_t2_II_bin_method_99p.csv")
HEADERS = None
os.chdir(src)
...
paramPath = "/Users/monicasilva/PycharmProjects/ProBCR/radiomics_kit/data_T2_II.yml"

list_images = sorted(glob.glob("*norm.nii"))
cases = list_images
...
for i in list_images:
    ...
    calculated_bins.append(len(out_bin) + 1)
    print("calculated_bins = ", len(out_bin) + 1)
